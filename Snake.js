...
Snake.prototype.render = function () {
    // 蛇头的渲染
    handleCell(this.body[0].row, this.body[0].col, (cell) => {
        cell.style.background = '#FA6A6A'
    });
    // 蛇身的渲染
    for (let i = 1; i < this.body.length; i++) {
        handleCell(this.body[i].row, this.body[i].col, (cell) => {
            cell.style.background = '#FA6A6A'
        });
    }
}

function handleCell(row, col, callback) {
    const table = document.querySelector('table')
    const cell = table.getElementsByTagName('tr')[row].getElementsByTagName('td')[col]
    callback(cell)
}
